...
export interface SurmeCamPageViewProps {
  galleryImages: string[]
  coverImage: string
}

/**
 * Sürme Cam Sistemleri sayfası (istemci görünümü)
 */
export function SurmeCamPageView({ galleryImages, coverImage }: SurmeCamPageViewProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main>
        ...
        <SlidingGlassHero key={`hero-${language}`} coverImage={coverImage} />
 ...
 */
function SlidingGlassHero({ coverImage }: { coverImage: string }) {
  const { t, language } = useLanguage()
  ...
  return (
    <CamSistemleriHero 
      ...
      imageSrc={publicAssetSrc(coverImage)}
    />
  )
}
